<script>
    ...
    import {autoname1, autoname2, autoname3, autoname4} from './_auto';
    ...
    const setData = (data) => {
        ...
        try {
            ...
            addToast(`setData error: ${error}`,'warn', 5000);
        } finally {
            loading = false;
            addToast(`editing cssVar id: ${id}`,'info', 5000);
        }
    }
    ...
    const updateData = async (id) => {
        ...
        try {
            ...
            .eq('id', id)
            if (error) throw error;
            addToast(`update data id: ${id}`,'prim', 2000);
        } catch (error) {
            addToast(`setData error: ${error}`,'warn', 5000);
        } finally {
            loading = false;
    ...
    const deleteData = async (id) => {
        ...
        try {
            const { data, error } = await supabase
                ...
                .eq('id', id)
            if (error) throw error;
            addToast(`delete data id: ${id}`,'warn', 5000);
        } catch (error) {
            addToast(`setData error: ${error}`,'warn', 5000);
        } finally {
            loading = false;
    ...
    <article class="py-8">
            ...
            <form on:submit|preventDefault={handleSubmit}>
                ...
                <div class="mb-1">
                    ...
                    <input class="input" bind:value={name1} name="name1" list="name1" type="text" placeholder="VName1">
                </div>
                <div class="mb-1">
                    <label class="label" for="name2">VName2</label>
                    <input class="input" bind:value={name2} name="name2" list="name2" type="text" placeholder="VName2">
                </div>
                <div class="mb-1">
                    <label class="label" for="name3">VName3</label>
                    <input class="input" bind:value={name3} name="name3" list="name3" type="text" placeholder="VName3">
                </div>
                <div class="mb-1">
                    <label class="label" for="val">Value</label>
                    <input class="input" bind:value={val} name="val" list="name4" type="text" placeholder="Value">
                </div>
                <div class="mb-1">
